(ns optumrx.main
  "Kotodama WASM entrypoint for the Optumrx clean-room actor (L4) — Clojure port.
  ...
  facts (`optumrx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "optumrx")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "optumrx_pre"
    :fields [:memberId :drugId :quantity :refills :status] :required [:quantity :refills]
    :coerce {:quantity :int :refills :int} :refs {:memberId "Member" :drugId "Drug"}}
   {:entity "Claim" :plural "claims" :id-prefix "optumrx_cla"
    :fields [:prescriptionId :pharmacyId :amount :status] :required [:amount :status]
    :coerce {:amount :float} :refs {:prescriptionId "Prescription" :pharmacyId "Pharmacy"}}
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "optumrx_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "optumrx_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "optumrx_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "optumrx_pri"
    :fields [:prescriptionId :status :decidedAt] :required [:status :decidedAt]
    :coerce {} :refs {:prescriptionId "Prescription"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"optumrx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "optumrx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :optumrx-compat/ready)
